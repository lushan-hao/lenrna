...
const PopFunction = (props: TimePickProps & PopFunctionProps) => {
  ...
  const disabledHoursList = useMemo(() => {
    ...
    const arr = minuteValue !== '00' ? ['24'] : minuteValue === '00' ? ['00'] : [];
  ...
  const disabledMinutesList = useMemo(() => {
    ...
    if (disabledMinutes && Array.isArray(disabledMinutes(Number(hourValue)))) {
      return disabledMinutes(Number(hourValue)).reduce((acc: string[], item: number) => {
        return acc.concat([`${item > 10 ? '' : 0}${item}`]);
      }, []);
    }
    return [];
        ...
        <Fragment>
          ...
          <ul>
            {MinuteList.map((item: string) => {
              ...
              if (disabledMinutesList.findIndex((p: string) => p === item) > -1) {
                currentClassName = 'disabledTime';
              }
              return (
                <li
            ...
            <Button
              ...
              onClick={() => {
                const localTimeString =
                  moment().format('HH:mm') === '00:00' ? '24:00' : moment().format('HH:mm');
                const TimeList = localTimeString.split(':');
                // 分钟步长选择
                const newTimeList = LocalTime({
                  List: TimeList,
                  minuteStep,
                  MinuteList: MinuteList?.filter((p) => !disabledMinutesList.find((e) => e === p)),
                  hourStep,
                  hourList: hourList?.filter((p) => !disabledHoursList.find((e) => e === p)),
                });
                // console.log(TimeList, 'TimeList');
                AssignmentData(`${newTimeList[0]}:${newTimeList[1]}` ?? localTimeString);
              }}
            >
              此时
            </Button>
